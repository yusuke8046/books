...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let allLoadedBooks = []; // スプレッドシートから読み込んだ全蔵書データ
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('sw_260812.js').catch(err => {
  ...
  async function fetchSheetBooks() {
    ...
    try {
      ...
      if (data.status === 'success' && data.books) {
        allLoadedBooks = data.books.reverse(); // 最新登録順
